...
class VOC2007DetectionTiny(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if split=="train":
          self.instances = json.load(
            open(os.path.join(dataset_dir, "pred_labels_complete.json")))
        elif split=="val":
          self.instances = json.load(
            open(os.path.join(dataset_dir, "gt_labels.json"))
        )
    ...
    def __getitem__(self, index: int):
        ...
        image_path_split=image_path.split("/")[-1]
        image_path=os.path.join(self.dataset_dir,"datasets/VOCdevkit/VOC2012/JPEGImages/"+image_path_split)
        ...
        if gt_boxes.shape[0]==0:
          ...
          #print("here_box") 
          #gt_boxes=-1*torch.ones((1,4)) 
        #print("The shape of the GT box is",gt_boxes.shape)
        #gt_boxes = gt_boxes * 224/400
        gt_classes = torch.Tensor([self.class_to_idx[inst["name"]] if inst["name"]!=[] else -1 for inst in ann])

        gt_classes = gt_classes.unsqueeze(1)  # (N, 1)
        
        #gt_boxes = torch.tensor([[int(inst["bndbox"]["xmin"]) ,int(inst["bndbox"]["ymin"]), int(inst["bndbox"]["xmax"]), int(inst["bndbox"]["ymax"])] for inst in annotation['object']])
        #gt_classes = torch.Tensor([self.class_to_idx[inst["name"]] for inst in annotation['object']])
        #print(gt_classes)
        #gt_classes = gt_classes.unsqueeze(1)  # (N, 1)
        ...
        #print("here1",normalize_tens[None, :].shape)
        ...
        gt_boxes = torch.cat(
            [gt_boxes, torch.zeros(abs(40 - len(gt_boxes)), 5).fill_(-1.0)]
        )
...
def train_detector(
    ...
    for _iter in range(max_iters):
        ...
        #print("debughere",next(train_loader))
...
def inference_with_detector(
    ...
    for iter_num, test_batch in enumerate(test_loader):
        ...
        if output_dir is not None:
            ...
            eecs545.utils.detection_visualizer(
